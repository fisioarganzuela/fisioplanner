...
import { BLOCKS, currentQuarter } from "@/lib/pilates";
...
function ObjectivesPage() {
  ...
  const now = new Date();
  const [editing, setEditing] = useState<Partial<Objective>>({ year: now.getFullYear(), quarter: currentQuarter(now), title: "", description: "" });
  ...
  const save = async () => {
    ...
    setOpen(false);
    setEditing({ year: now.getFullYear(), quarter: currentQuarter(now), title: "", description: "" });
    load();
  };
  ...
  return (
    <div className="p-8 max-w-5xl">
      <PageHeader
        ...
        subtitle="Una planificación anual de Pilates máquina dividida en 4 bloques."
        actions={
          <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEditing({ year: now.getFullYear(), quarter: currentQuarter(now), title: "", description: "" }); }}>
            <DialogTrigger asChild><Button className="rounded-full"><Plus className="size-4" /> Nuevo objetivo</Button></DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>{editing.id ? "Editar" : "Nuevo"} objetivo trimestral</DialogTitle></DialogHeader>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    <Label>Trimestre / Bloque</Label>
                    <Select value={String(editing.quarter ?? 1)} onValueChange={(v) => setEditing({ ...editing, quarter: Number(v) })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {[1,2,3,4].map((q) => <SelectItem key={q} value={String(q)}>Q{q} · {BLOCKS[q].title}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground -mt-2">{BLOCKS[editing.quarter ?? 1].months} · {BLOCKS[editing.quarter ?? 1].focus}</p>
                <div className="space-y-2"><Label>Título</Label><Input value={editing.title ?? ""} onChange={(e) => setEditing({ ...editing, title: e.target.value })} placeholder="Ej. Asentar el powerhouse en el grupo de iniciación" /></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[1,2,3,4].map((q) => {
          ...
          const matching = items.filter((o) => o.quarter === q);
          return (
            <Card key={q} className="p-5">
              <div className="flex items-baseline justify-between">
                <div>
                  <div className="text-xs uppercase tracking-wide text-primary font-semibold">Bloque {q}</div>
                  <div className="font-display text-xl font-semibold mt-0.5">{block.title}</div>
                  <div className="text-xs text-muted-foreground mt-0.5">{block.months}</div>
                </div>
              </div>
              <p className="text-sm text-muted-foreground mt-3">{block.focus}</p>
